...
export const getUserBalances = query({
  handler: async (ctx) => {
    ...
    let youAreOwed = 0;  // total amount others owe the users
    const balanceByUser = {}; 
    ...
    for (const e of expenses) {
      ...
      if (isPayer) {
        for (const s of e.splits) {
          ...
          youAreOwed += s.amount;
          balanceByUser[s.userId] = balanceByUser[s.userId] || { owed: 0, owing: 0 };
          balanceByUser[s.userId].owed += s.amount;
        }
      } else if (mySplit && !mySplit.paid) {
        youOwe += mySplit.amount;
        balanceByUser[e.paidByUserId] = balanceByUser[e.paidByUserId] || { owed: 0, owing: 0 };
    ...
    for (const s of settlements) {
      if (s.paidByUserId === user._id) {
        youOwe -= s.amount;
        balanceByUser[s.receivedByUserId] = balanceByUser[s.receivedByUserId] || { owed: 0, owing: 0 };
        balanceByUser[s.receivedByUserId].owing -= s.amount;
      } else {
        youAreOwed -= s.amount;
         balanceByUser[s.paidByUserId] = balanceByUser[s.paidByUserId] || { owed: 0, owing: 0 };
