...
export const seedDatabase = mutation({
  ...
  handler: async (ctx : MutationCtx) => {
    ...
    const users = await ctx.db.query("users").collect() as User[];
...
async function createGroups(ctx: MutationCtx, users : User[]): Promise<Group[]> {
  const now = Date.now();
  ...
  return await Promise.all(
    groupIds.map(async (id) => {
      ...
      if (!group) throw new Error(`Group not found for id: ${id}`);
      return { ...group, _id: id };
    })
  );
}
...
async function createOneOnOneExpenses(ctx:MutationCtx, users: User[]): Promise<Expense[]> {
  const now = Date.now();
  ...
  return await Promise.all(
    expenseIds.map(async (id) => {
      ...
      if (!expense) throw new Error(`Expense not found for id: ${id}`);
...
async function createGroupExpenses(ctx:MutationCtx, users:User[], groups:Group[]): Promise<Expense[]> {
  const now = Date.now();
  const oneWeekAgo = now - 7 * 24 * 60 * 60 * 1000;
  const twoWeeksAgo = now - 14 * 24 * 60 * 60 * 1000;
...
async function createSettlements(
  ...
  return await Promise.all(
    settlementIds.map(async (id) => {
      ...
      if (!settlement) throw new Error(`Settlement not found for id: ${id}`);
